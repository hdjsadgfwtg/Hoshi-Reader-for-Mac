// selection.js — Android port
// Bridge: webkit.messageHandlers.X.postMessage() → window.HoshiBridge.X()
...
window.hoshiSelection = {
        ...
        if (document.caretPositionFromPoint) {
            ...
            while (node = walker.nextNode()) {
                for (let i = 0; i < node.textContent.length; i++) {
                    ...
                    if (this.inCharRange(range, x, y)) { range.collapse(true); return range; }
                }
            }
    ...
    getSentence(startNode, startOffset) {
        ...
        for (let i = 0; i < sentence.length; i++) {
            ...
            if (openBrackets.has(ch)) { stack.push(ch); }
                ...
                if (stack.length > 0 && this.brackets[stack[stack.length-1]] === ch) { stack.pop(); }
                else { unmatchedClose.push(ch); }
            }
        }
        let startSlice = 0;
        while (stack.length > 0 && startSlice < sentence.length - 1) {
            if (stack[0] === sentence[startSlice]) { stack.shift(); } else break;
            startSlice++;
        }
    ...
    selectText(x, y, maxLength) {
        ...
        try {
            window.HoshiBridge.textSelected(JSON.stringify({
                text,
                sentence,
...
document.addEventListener('selectionchange', () => {
    ...
    try { window.HoshiBridge.selectionState(hasSelection); } catch {}
});
