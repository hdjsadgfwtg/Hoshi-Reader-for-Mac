...
function openExternalLink(url) {
    window.HoshiBridge.openLink(url);
}
...
async function mineEntry(expression, reading, frequencies, pitches, rules, matched, entryIndex, popupSelectionText) {
    ...
    return window.HoshiBridge.mineEntry(mineData);
}
...
function playWordAudio(audioUrl) {
    try {
        window.HoshiBridge.playWordAudio(JSON.stringify({
            url: audioUrl,
...
function createEntryHeader(entry, idx) {
    ...
    const mineButton = el('button', {
        ...
        onclick: async () => {
            ...
            const checkDuplicate = () => {
                const wasAdded = window.HoshiBridge.duplicateCheck(expression);
    ...
    try {
        const isDuplicate = window.HoshiBridge.duplicateCheck(expression);
        if (isDuplicate) {
